...
def count_all_possible_options(speakers, curr_sequence, n_sequences=[0], n_good_sequences=[0]):
    """ 2) Полный перебор всевозможных вариантов """

    if len(speakers) == 0:
        n_sequences[0] += 1
        if curr_sequence.index("A") < curr_sequence.index("B") < curr_sequence.index("V"):
